...
window.addEventListener("DOMContentLoaded", () => {
  ...
  //Make the upload info message visible
  const uploadError = document.getElementById("uploadError");
  if (uploadError) {
    //Style it as an info message rather than an error
    // uploadError.style.color = 'var(--neutral-300)';
    uploadError.style.fontSize = "12px";
    uploadError.style.display = "flex";
    // uploadError.style.alignItems = 'center';
    // uploadError.style.justifyContent = 'center';
    uploadError.style.marginTop = '8px';

    //Change the text slightly to make it more informative
    uploadError.innerHTML =
      '<img src="assets/images/icon-info.svg" class="info" alt=""> Please upload your photo (JPG or PNG, max size: 500KB).';
  }
});
...
function resetUploadArea() {
  ...
  // Reset the upload error/info message back to its informative state
  uploadError.innerHTML =
    '<img src="assets/images/icon-info.svg" class="info" alt=""> Please upload your photo (JPG or PNG, max size: 500KB).';
  uploadError.classList.remove("error");
  // uploadError.style.color = "var(--neutral-300)";
...
function downloadTicket() {
  ...
  backgroundContainer.style.width = ticketElement.offsetWidth + 40 + "px";
  backgroundContainer.style.height = ticketElement.offsetHeight + 40 + "px";

  // Add the ticket to the background container
  backgroundContainer.appendChild(ticketClone);

  // Add the container to  temp element
